...
class RNA_counts:
    ...
    def __init__(self, data, labels=[0, 0], cells_axis=0, barcodes=None):
        ...
        self.counts = np.ascontiguousarray(
            self.data
        )  # store counts matrix as counts attribute (no labels, np.array format)

        if barcodes is not None:  # if barcodes df provided, merge with data
            data_coded = self.data.merge(
                barcodes, left_index=True, right_index=True, how="left"
            )
            data_coded = data_coded.astype({"Barcode": "category"})
            self.data_coded = (
                data_coded  # create 'coded' attribute that has data and barcodes
            )
            self.barcodes = data_coded[
                "Barcode"
            ]  # make barcodes attribute pd.Series for passing to other classes
    ...
    def distance_matrix(self, transform=None, ranks="all", **kwargs):
        ...
        ranks_i = self.barcodes.value_counts()[
            self.barcodes.value_counts()
            .rank(axis=0, method="min", ascending=False)
            .isin(ints)
        ].index
        ranks_counts = transformed[
            np.array(self.barcodes.isin(list(ranks_i) + IDs))
        ]  # subset transformed counts array
    ...
    def barcode_distance_matrix(self, ranks, transform=None, **kwargs):
        ...
        ranks_0 = transformed[
            np.array(self.barcodes.isin(list(ranks[0])))
        ]  # subset transformed counts array to first barcode ID
        ranks_1 = transformed[
            np.array(self.barcodes.isin(list(ranks[1])))
        ]  # subset transformed counts array to second barcode ID
    ...
    def knn_graph(self, k, **kwargs):
        ...
        return kneighbors_graph(
            self.distance_matrix(**kwargs), k, mode="connectivity", include_self=False
        ).toarray()
    ...
    def barcode_counts(self, IDs="all"):
        ...
        return self.barcodes.value_counts()[
            self.barcodes.value_counts().index.isin(IDs)
        ]
    ...
    def log2_norm(self, norm="l1", ranks="all"):
        ...
        if not norm:
            ...
            out = np.log2(
                normalize(self.counts, axis=1, norm=norm) + 1
    ...
    def from_file(cls, datafile, labels=[0, 0], cells_axis=0, barcodefile=None):
        ...
        """
        filetype = os.path.splitext(datafile)[
            1
        ]  # extract file extension to save as metadata

        if filetype == ".zip":  # if compressed, open the file and update filetype
            zf = zipfile.ZipFile(datafile)
            datafile = zf.open(
                os.path.splitext(datafile)[0]
            )  # update datafile with zipfile object
            filetype = os.path.splitext(os.path.splitext(datafile)[0])[
                1
            ]  # update filetype
        ...
        if filetype == ".csv":  # read comma-delimited tables
            ...
            data = pd.read_csv(
                datafile, header=labels[1], index_col=labels[0], sep="\t"
            )
        ...
        if filetype == ".gz":  # if file is g-zipped, read accordingly
            ...
            if filetype == ".csv":
                data = pd.read_csv(
                    gzip.open(datafile), header=labels[1], index_col=labels[0]
                )

            elif filetype in (".txt", ".tsv"):
                data = pd.read_csv(
                    gzip.open(datafile), header=labels[1], index_col=labels[0], sep="\t"
                )

        if barcodefile:  # if barcodes provided, read in file
            barcodes = pd.read_csv(
                barcodefile, index_col=None, header=None, names=["Barcode"]
            )
    ...
    def drop_set(cls, counts_obj, drop_index, axis, num=False):
        ...
        if counts_obj.barcodes is not None:
            ...
            codes[
                "Cell Barcode"
            ] = codes.index  # make barcodes mergeable when calling cls()
        ...
        if not num:
            return cls(
                counts_obj.data.drop(drop_index, axis=axis),
                labels=[counts_obj.cell_labels, counts_obj.gene_labels],
                barcodes=codes,
            )

        elif axis == 1:
            return cls(
                counts_obj.data.drop(counts_obj.data.columns[drop_index], axis=axis),
                labels=[counts_obj.cell_labels, counts_obj.gene_labels],
                barcodes=codes,
            )
            ...
            return cls(
                counts_obj.data.drop(counts_obj.data.index[drop_index], axis=axis),
                labels=[counts_obj.cell_labels, counts_obj.gene_labels],
                barcodes=codes,
            )
    ...
    def keep_set(cls, counts_obj, keep_index, axis, num=False):
        ...
        if not num:
            if axis == 0:
                return cls(
                    counts_obj.data.loc[keep_index, :],
                    labels=[counts_obj.cell_labels, counts_obj.gene_labels],
                    barcodes=codes,
                )
            ...
            if axis == 0:
                return cls(
                    counts_obj.data.iloc[keep_index, :],
                    labels=[counts_obj.cell_labels, counts_obj.gene_labels],
                    barcodes=codes,
                )
    ...
    def downsample_rand(cls, counts_obj, n_cells, seed=None):
        ...
        return cls(
            counts_obj.data.iloc[cells_out],
            labels=[counts_obj.cell_labels, counts_obj.gene_labels],
            barcodes=codes,
        )
    ...
    def downsample_proportional(cls, counts_obj, clu_membership, n_cells, seed=None):
        ...
        """
        np.random.seed(seed)  # set seed for reproducible sampling if desired
        IDs, clu_counts = np.unique(
            clu_membership, return_counts=True
        )  # get cluster IDs and number of cells in each

        cells_out = np.array([])  # initialize array of output cell indices
        for ID, count in zip(IDs, clu_counts):
            clu_num = (
                int(count / clu_counts.sum() * n_cells) + 1
            )  # number of cells to sample for given cluster
            cells_out = np.append(
                cells_out,
                np.random.choice(
                    np.where(clu_membership == ID)[0], clu_num, replace=False
                ),
            )
    ...
    def var_select(cls, counts_obj, n_features):
        ...
        """
        v = (
            counts_obj.data.var(axis=0).nlargest(n_features).index
        )  # get top n variant gene IDs
...
class DR:
        ...
        if barcodes is not None:
            if isinstance(barcodes, pd.DataFrame):
                self.barcodes = barcodes.iloc[
                    :, 0
                ]  # maintain given barcode information as pd.Series
            else:
    ...
    def silhouette_score(self):
        ...
        return silhouette_score(
            self.results, self.clu.membership
    ...
    def cluster_counts(self):
        ...
        for ID, count in zip(IDs, counts):
            print(
                "{} cells in cluster {} ({} %)\n".format(
                    count, ID, np.round(count / counts.sum() * 100, 3)
                )
            )
    ...
    def plot_clusters(self):
        ...
        ax[0].scatter(
            self.results[self.clu.clusters, 0],
            self.results[self.clu.clusters, 1],
            s=90,
            c="red",
        )
        ax[1].scatter(
            self.results[:, 0], self.results[:, 1], s=75, alpha=0.7, c=self.clu.density
        )
        ax[2].scatter(
            self.results[:, 0],
            self.results[:, 1],
            s=75,
            alpha=0.7,
            c=self.clu.membership,
            cmap="plasma",
        )
        IDs, counts = np.unique(
            self.clu.membership, return_counts=True
        )  # get cluster counts and IDs
        bbox_props = dict(
            boxstyle="round", fc="w", ec="0.5", alpha=0.9
        )  # set up annotate box
        # add percentages of each cluster to plot
        for _, count, x, y in zip(
            IDs,
            counts,
            self.results[self.clu.clusters, 0],
            self.results[self.clu.clusters, 1],
            ...
            ax[2].annotate(
                "{} %".format(np.round(count / counts.sum() * 100, 2)),
                xy=(x, y),
                ha="center",
                va="center",
                size=12,
    ...
    def plot_barcodes(self, ranks="all", save_to=None, figsize=(5, 5)):
        ...
        if ranks == "all":
            ...
            ints = [x for x in ranks if type(x) == int]  # pull out rank values
            IDs = [
                x for x in ranks if type(x) == str
            ]  # pull out any specific barcode IDs
            ranks_i = self.barcodes.value_counts()[
                self.barcodes.value_counts()
                .rank(axis=0, method="min", ascending=False)
                .isin(ints)
            ].index
            ranks_codes = self.barcodes[
                self.barcodes.isin(list(ranks_i) + IDs)
            ]  # subset barcodes series
            ranks_results = self.results[
                self.barcodes.isin(list(ranks_i) + IDs)
            ]  # subset results array
            sns.scatterplot(
                self.results[:, 0],
                self.results[:, 1],
                s=75,
    ...
    @classmethod
    def from_file(
        cls, datafile, labels=[None, None], cells_axis=0, name="Dim", barcodefile=None
    ):
        """
        initialize object from outside file (datafile)
...
class fcc_PCA(DR):
    ...
    def plot_PCA(self, color=None, save_to=None, figsize=(10, 5)):
        ...
        plt.subplot(122)
        plt.plot(
            np.cumsum(np.round(self.fit.explained_variance_ratio_, decimals=3) * 100)
        )
...
class fcc_tSNE(DR):
    ...
    def __init__(self, matrix, perplexity, seed=None, barcodes=None, **kwargs):
        ...
        self.fit = TSNE(perplexity=self.perplexity, random_state=seed, **kwargs).fit(
            self.input
        )
...
class fcc_FItSNE(DR):
    ...
    def __init__(
        self, matrix, perplexity, seed=-1, barcodes=None, clean_workspace=True
    ):
        DR.__init__(self, matrix=matrix, barcodes=barcodes)  # inherits from DR object
        self.name = "FIt-SNE"
        self.perplexity = perplexity  # store tSNE perplexity as metadata
...
class fcc_UMAP(DR):
    ...
    def __init__(self, matrix, perplexity, seed=None, barcodes=None, **kwargs):
        ...
        self.fit = UMAP(n_neighbors=self.perplexity, random_state=seed, **kwargs).fit(
            self.input
        )
        self.results = self.fit.fit_transform(self.input)
